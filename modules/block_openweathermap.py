...
class BlockOpenWeatherMap(BlockBase):
    ...
    def init(self, fileName):
        ...
        if self._key is None:    raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "Key"))
        if self._time is None:   raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "UpdateTime"))
        if self._folder is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "Folder"))

        if self._iconScale is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "IconScale"))
        if self._iconPos is None:   raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "IconPos"))

        if self._weatherTypePos is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WeatherTypePos"))
        if self._temperaturePos is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "TemperaturePos"))
        if self._humidityPos is None:    raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "HumidityPos"))
        if self._pressurePos is None:    raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "PressurePos"))
        if self._windPos is None:        raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WindPos"))

        if wtName is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WeatherTypeFontName"))
        if tName is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "TemperatureFontName"))
        if hName is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "HumidityFontName"))
        if pName is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "PressureFontName"))
        if wName is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WindFontName"))

        if wtSize is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WeatherTypeFontSize"))
        if tSize is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "TemperatureFontSize"))
        if hSize is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "HumidityFontSize"))
        if pSize is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "PressureFontSize"))
        if wSize is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WindFontSize"))

        if wtBold is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WeatherTypeFontBold"))
        if tBold is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "TemperatureFontBold"))
        if hBold is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "HumidityFontBold"))
        if pBold is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "PressureFontBold"))
        if wBold is None:  raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WindFontBold"))

        if wtItalic is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WeatherTypeFontItalic"))
        if tItalic  is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "TemperatureFontItalic"))
        if hItalic  is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "HumidityFontItalic"))
        if pItalic  is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "PressureFontItalic"))
        if wItalic  is None: raise Exception(TEXT_EXCEPTION_NOT_FOUND.format("OpenWeatherMapBlock", "WindFontItalic"))
