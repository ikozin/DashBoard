...
class AlarmTimeFileBase(AlarmTimeBase):
    ...
    def init(self, config_section, mod_list):
        ...
        self._fileName = config_section.get("File")
        ...
        if len(self._fore_color) != 3:
            raise ExceptionFormat(config_section.name, "foreground_color")
        if len(self._back_color) != 3:
            raise ExceptionFormat(config_section.name, "background_color")
